...
plotAllTemporalComparisons <- function(Benchmark = this_benchmark, all_comparisons = all_comparisons){
  ...
  names(grid.names) <- paste0("(", ICOS_grid$ICOS_Lon, ",", ICOS_grid$ICOS_Lat, ")")
  }
  ...
  if (Benchmark@datasets[[1]]@source@format@id == "FLUXNET"){
    ...
    grid.names <- FLUXNET_grid$Name
    names(grid.names) <- paste0("(", FLUXNET_grid$GUESS_Lon, ",", FLUXNET_grid$GUESS_Lat, ")")
    }
