...
getField_FLUXNET <- function(source,
  ...
  if (length(CH4.files) != 0) {
    ...
    for (i in 1:length(CH4.files)) {
      ...
      # subset the site metadata for this site only (might make the code a little faster)
      ...
      elevation <-  as.numeric(as.character(this.siteinfo.data$DATAVALUE[this.siteinfo.data$VARIABLE == "LOCATION_ELEV"][1]))
  ...
  if (length(FLUXNET2015.files) != 0) {
    ...
    for (i in 1:length(FLUXNET2015.files)) {
      ...
      # Determine the longitude, latitude, elevation and IGBP classification
      # NOTE: Some sites have more than one location recorded in the site description file, due to reasons like the tower being moved
      # or the meteorological instrumentation being set located slightly further away.
      # The above applies to sites in the 2015 release, I have no idea how it applies to the CH4 community product.  
      # I have adopted the convention of just taking the first location in the file
