...
Data.year.mean <- list()
  processDataSource(all_datasets = all_GUESS_datasets, input, benchmark_name,spatial.extent = spatial.extent, simulation = 1, dataset = 1)
...
if (do_profound){
benchmark_name <- "Profound_csv"

Data.year.mean <- processDataSource(all_datasets = all_SITE_datasets, input, benchmark_name, simulation = 2, dataset = 1)

this_benchmark <- createBenchmark(benchmark_name, input, simulation = 2, Data.year.mean, spatial.extent)

field_lists <- DGVMBenchmarks::getAllFields(this_benchmark,all_SITE_simulation_Sources_list)
all_sim_full <- field_lists[[1]]
all_Fields_list <- field_lists[[2]]
...
all_comparisons <- DGVMBenchmarks::fullTemporalComparison(benchmark = this_benchmark, all_ts = all_Fields_list,       new_model = all_SITE_simulation_Sources_list[[1]]@name, old_model = all_SITE_simulation_Sources_list[[2]]@name)
...
if(length(this_benchmark@datasets[[1]]) != 0){Profound_table <- rbind(Profound_table,
                     DGVMBenchmarks::makeMetricTable(benchmark = this_benchmark,
                                      all_comparisons_list = all_comparisons,
                                      simulation_sources = all_GUESS_simulation_Sources_list))}
}
...
if (do_FLUXNET_GPP){
  # Define which benchmark to do
benchmark_name <- "FLUXNET_GPP"

# Process your dataset
Data <- processDataSource(all_FLUXNET_datasets, input, spatial.extent = spatial.extent, benchmark_name, simulation = 6, dataset = 1)

Data <- correctFLUXNETDataset(Data)

# Build your current benchmark from input YAML file  
this_benchmark <- createBenchmark(benchmark_name, input, simulation = 4, Data, spatial.extent)
...
## Extracts statistical metrics from your fields.
if(length(this_benchmark@datasets[[1]]) != 0){FLUXNET_table <- rbind(FLUXNET_table,
                     DGVMBenchmarks::makeMetricTable(benchmark = this_benchmark,
                                      all_comparisons_list = all_comparisons,
                                      simulation_sources = all_GUESS_simulation_Sources_list))}
}
...
plotFLUXNETScatter(this_benchmark,all_comparisons,fill="station")
```

## GPP per Land cover
```{r per lc flux GPP, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
plotFLUXNETScatter(this_benchmark,all_comparisons,fill="landcover")
```
...
Data <- correctFLUXNETDataset(Data)
# Build your current benchmark from input YAML file  
this_benchmark <- createBenchmark(benchmark_name, input, simulation = 5, Data, spatial.extent)
