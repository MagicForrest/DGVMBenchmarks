...
# Make table for harvest
do_harvest <- as.logical(input[["Switches"]][["do_harvest"]])
...
```{r}
benchmark_name <- "GROWSTOCK"

# Load the ncdf4 package
nc_old <- nc_open("C:/Users/Admin/Documents/tellus/WUR/WUR_test.nc")

# Read the existing dimensions (northing and easting)
northing_vals <- ncvar_get(nc_old, "northing")  # Values for the northing dimension
easting_vals <- ncvar_get(nc_old, "easting")    # Values for the easting dimension

# Create new dimensions with the renamed labels (lat and lon)
lat_dim <- ncdim_def(name = "lat", units = "degrees_north", vals = northing_vals)
lon_dim <- ncdim_def(name = "lon", units = "degrees_east", vals = easting_vals)

# Get the variable you want to copy (assuming 2D variables indexed by easting/northing)
existing_var <- nc_old$var[["variable"]]  # Replace 'your_variable' with actual variable name

# Define the new variable with the renamed dimensions
new_var_def <- ncvar_def(name = existing_var$name, 
                         units = existing_var$units, 
                         dim = list(lon_dim, lat_dim))

# Create a new NetCDF file with the renamed dimensions
nc_new <- nc_create("C:/Users/Admin/Documents/tellus/WUR/WUR_test_modified.nc", 
                    vars = list(new_var_def))

# Write the data from the original variable to the new file
data_to_write <- ncvar_get(nc_old, "variable")  # Replace 'your_variable' with actual variable name
ncvar_put(nc_new, new_var_def, data_to_write)

# Close both the old and new NetCDF files
nc_close(nc_old)
nc_close(nc_new)
...
plotSpatialField(Benchmark = this_benchmark, all_Fields_list = all_Fields_list, opt.layer = 'vegc_europe_sc_2020_2025')```
...
storm_table<-plotStormScatter(this_benchmark, all_sim_full, do_plots, anno.hjust = 110)
...
if (do_storm){
  ...
  mutate(
    ...
    `Reineke Before Calibration` = round(`Reineke Before Calibration`, 2),
    `Reineke After Calibration` = round(`Reineke After Calibration`, 2),
    `No Thinning Before Calibration` = round(`No Thinning Before Calibration`, 2),
    `No Thinning After Calibration` = round(`No Thinning After Calibration`, 2)
  ) %>%
  rename(
    Country = `Country`,
...
# Harvest Table {.tabset}

## Net annual increment
```{r Harvest,echo=FALSE, message=FALSE, results='hide', warning=FALSE}
if (do_harvest){
benchmark_name <- "NAI" 

this_benchmark <- createBenchmark(benchmark_name, input, Datasets = list(), spatial.extent)

field_lists <- DGVMBenchmarks::getAllFields(this_benchmark, all_simulation_Sources_list)
all_sim_full <- field_lists[[1]]
all_Fields_list <- field_lists[[2]]

Harvest_table <- bind_rows(Harvest_table, HarvestTable("NAI",this_benchmark,all_sim_full))}
```

## Harvest
```{r Harvest1,echo=FALSE, message=FALSE, results='hide', warning=FALSE}
if (do_harvest){
benchmark_name <- "Harvest" 
...
Harvest_table <- bind_rows(Harvest_table, HarvestTable("Total Harvest",this_benchmark,all_sim_full))}
```

## Harvest Table
```{r Harvest2,echo=FALSE}
if (do_harvest){
Harvest_table <- Harvest_table %>%
  mutate(across(where(is.numeric), ~ round(., 2)))  # Round to 2 decimals


ft <- flextable(Harvest_table)
ft <- merge_v(ft, j = "Parameter")
ft <- colformat_num(ft, j = c("Czechia","Finland","France","Germany","Netherlands","Poland","Spain","Sweden","Switzerland" ), digits = 2, big.mark = "")
ft}
```
...
merged_data <- merge(all_Fields_list[[1]]@data, ICOS_grid, 
                                by = c("Code", "Name"), 
                                all = TRUE)
all_Fields_list[[1]]@data[, `:=`(Lat = merged_data$GUESS_Lat, Lon = merged_data$GUESS_Lon)]

all_Fields_list[[1]]@data <- all_Fields_list[[1]]@data[!is.na(Lat) & !is.na(Lon)]

all_Fields_list[[1]]@data <- all_Fields_list[[1]]@data %>% 
  filter(Name != "Soroe") %>%
  filter(Name != "Auchencorth Moss") %>%
  filter(Name != "Grignon") 

data <- all_Fields_list[[1]]@data

# Remove the first year for each unique Name
filtered_data <- data %>%
  group_by(Name) %>%
  filter(Year != min(Year)) %>%
  ungroup()  # Ungroup afterwards

# Assign the filtered data back
all_Fields_list[[1]]@data <- as.data.table(filtered_data)
...
#code <- unique(all_Fields_list[[1]]@data$Code)

all_Fields_list[[2]]@data <- all_Fields_list[[2]]@data[!( (Lat == 61.25 & Lon == 11.75) | (Lat == 64.25 & Lon == 19.75) ), ]
...
max_NEE <- max(all_Fields_list[[2]]@data$NEE, na.rm = TRUE)

# Find the row(s) where NEE is greater than the maximum value
rows_greater_than_max <- all_Fields_list[[2]]@data[all_Fields_list[[2]]@data$NEE == max_NEE, ]
print(rows_greater_than_max)
all_Fields_list[[2]]@data[all_Fields_list[[2]]@data$NEE == max_NEE, ] <- NA
