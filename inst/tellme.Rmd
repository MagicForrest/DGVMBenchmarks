...
if(do_global_LAI){
  ...
  if(length(LAI_data_Fields) == 0) {
    ...
    for(this_data_Field in this_benchmark@datasets) {
      ...
      # TODO, make this more flexible for multiple datasets
      data_LAI_MegaM2 <- areaWeightedTotal(all_LAI_Maps[[this_data_Field@source@name]])@data * M2_TO_MegaM2
    ...
    for(this_sim_Source in all_simulation_Sources_list) {
      ...
      if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
        ...
        # calculate and save the global total
        this_LAI_MegaM2 <- areaWeightedTotal(all_LAI_Maps[[this_sim_Source@name]])@data * M2_TO_MegaM2
...
if(do_Regrowth){
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
      ...
      # Boreal only
      regrowth_mod <- age_bins %>% group_by(new_bin) %>% summarise(AGcwood_kgCm2_med = median(Total),
                                                                   AGcwood_kgCm2_10 = quantile(Total, c(.10) ),
                                                                   AGcwood_kgCm2_90 = quantile(Total, c(.90) ),
                                                                   bin_centres =  floor(mean(Age)))
      
      regrowth_mod_full <- age_bins %>% group_by(new_bin, Biome) %>% summarise(AGcwood_kgCm2_med = median(Total),
                                                                               AGcwood_kgCm2_10 = quantile(Total, c(.10) ),
                                                                               AGcwood_kgCm2_90 = quantile(Total, c(.90) ),
                                                                               bin_centres =  floor(mean(Age)))
      
      #visualise aggregated output:
      plot(regrowth_mod$new_bin,
           regrowth_mod$AGcwood_kgCm2_med,
           ylim=c(0,35),col="red",xlim=c(0,16),ylab='',xlab='',
           xaxt = 'n',yaxt= 'n')
      
      abline(h=10,col="light grey")
      arrows(1:30, regrowth_mod$AGcwood_kgCm2_10, 1:30, regrowth_mod$AGcwood_kgCm2_90, length=0.00, angle=90, code=3,col= "dark green")
      ...
      plot(regrowth_obs$bin_num/10,
           regrowth_obs$AGcwood_kgCm2_med,
           ylim=c(0,35),col="red",xlim=c(0,16),ylab='',xlab='',
           xaxt = 'n',yaxt= 'n')
      
      arrows(regrowth_obs$bin_num/10, regrowth_obs$AGcwood_kgCm2_10, regrowth_obs$bin_num/10, regrowth_obs$AGcwood_kgCm2_90, length=0.00, angle=90, code=3,col= "red")
...
if(do_Regrowth){
  ...
  regrowth_plot <- ggplot(regrowth_full_dt, aes(x=bin_centres)) + geom_point(aes(y = AGcwood_kgCm2_med,
    ...
    facet_wrap(~Biome, ncol = 2, scales = "free_y" ) +
...
if(do_Global_Biomass){
  ...
  if(file.exists(full_file_path)) {
    ...
    # calculate the global total
    this_Biomass_total_PgC  <- areaWeightedTotal(this_data_Field)@data * KG_TO_PG
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
      ...
      # calculate the global total
      this_Biomass_total_PgC  <-areaWeightedTotal(all_Biomass_Maps[[this_sim_Source@name]])@data * KG_TO_PG
