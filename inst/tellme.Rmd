...
if(do_GOSIF_GPP){
  ...
  #### Now that we have read all the data, make all the comparisons
  ...
  for(this_simulation_Source in all_simulation_Sources_list) {
    ...
    suppressWarnings(suppressMessages(
      spatial_comparisons_list[[this_simulation_Source@id]] <- compareLayers(field1 = all_GPP_Map_Fields_list[[this_simulation_Source@id]],
                                                                             ...
                                                                             layers1 =  guess_var, layers2 =  guess_var, show.stats = FALSE)))
    # store the NME 
    ...
    suppressMessages(suppressWarnings(
      addArea(all_GPP_Map_Fields_list[[this_simulation_Source@id]], "m^2") %>%
        layerOp("*", c(guess_var, "Area"), "Area_summed") %>%
        selectLayers("Area_summed") %>%
        aggregateSpatial(method = "sum") -> total_GPP_PgC_Field
    ))
    
    total_GPP_PgC <- total_GPP_PgC_Field@data/1e+12
...
## Spatial patterns analysis {.tabset}
