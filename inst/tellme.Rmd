...
num_cols <- 1
map.overlay <- "world"  # set to NULL for faster plotting
...
##  Dinerstein 2017 ecoregions, no model output classification yet done
...
if(do_GOSIF_GPP){
  ...
  #### GOSIF GPP SETTINGS AND PARAMETERS ####
  ...
  makeSummaryLine <- function(benchmark, col_names) {
    ...
    summary_table_lines$Dataset <- benchmark@name
  ...
  if(file.exists(full_file_path)) {
    ...
    all_GPP_Trends[["GOSIF GPP"]] <- calcLinearTrend(GOSIF_full_Field)
    ...
    all_GPP_Maps[[this_data_Source@name]] <- aggregateYears(aggregateSubannual(GOSIF_full_Field, method = "sum", target = "Year"), method = "mean")
    
    # make the seasonal cycle for comparison (leave months andspatial)
    all_GPP_Seasonal[[this_data_Source@name]] <- aggregateYears(GOSIF_full_Field, method = "mean")
    
    # make and save the mean annual global GPP
    data_GPP_PgC <- areaWeightedTotal(all_GPP_Maps[[this_data_Source@name]])@data*KG_TO_PG
    all_GPP_totals[[this_data_Source@name]] <- data_GPP_PgC
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
      ...
      all_GPP_Maps[[this_sim_Source@id]] <- aggregateYears(aggregateSubannual(this_simulation_GPP, target = "Year", method = "sum"),  "mean")
      
      ### take yearly average and store for later use - very easy ###
      all_GPP_Seasonal[[this_sim_Source@id]] <- aggregateYears(this_simulation_GPP, "mean")
      ...
      all_GPP_Trends[[this_sim_Source@id]] <- this_GPP_Trend
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    suppressWarnings(
      spatial_comparisons_list[[this_sim_Source@id]] <- compareLayers(field1 = all_GPP_Maps[[this_sim_Source@id]],
                                                                      field2 = all_GPP_Maps[[this_data_Source@name]],
                                                                      layers1 =  this_benchmark@guess_var, 
                                                                      layers2 = this_benchmark@guess_var, 
                                                                      show.stats = FALSE))
    # store the NME 
    nme_scores_list[[this_sim_Source@name]] <- spatial_comparisons_list[[this_sim_Source@id]]@stats$NME
    
    #### GOSIF TRENDS COMPARISONS ####
    suppressWarnings(trend_comparisons_list[[this_sim_Source@id]] <- compareLayers(field1 = all_GPP_Trends[[this_sim_Source@id]],
                                                                                   field2 = all_GPP_Trends[[this_data_Source@name]],
                                                                                   layers1 = "Trend",
                                                                                   layers2 = "Trend", 
                                                                                   show.stats = FALSE))
    ...
    this_simulation_only_positive <- layerOp(all_GPP_Seasonal[[this_sim_Source@id]],
                                             operator = function(x){pmax(x,0)},
                                             layers = this_benchmark@guess_var, 
                                             new.layer = this_benchmark@guess_var)
    suppressWarnings(
      seasonal_comparisons_list[[this_sim_Source@id]] <- compareLayers(field1 = this_simulation_only_positive,
                                                                       field2 = all_GPP_Seasonal[[this_data_Source@name]],
                                                                       layers1 = this_benchmark@guess_var,
                                                                       layers2 = this_benchmark@guess_var, 
                                                                       do.seasonality = TRUE, 
                                                                       show.stats = FALSE))
    ...
    #### GLOBAL GPP SUMS ####
    total_GPP_PgC <- areaWeightedTotal(all_GPP_Maps[[this_sim_Source@id]])@data*KG_TO_PG
    all_GPP_totals[[this_sim_Source@name]] <- total_GPP_PgC
...
QUESTION: Should we also show trends *restricted* to where they are significant?
...
if(do_GOSIF_GPP) {
  suppressWarnings(print(
    plotSpatialComparison(seasonal_comparisons_list, 
                          do.phase = TRUE,
                          type = "values",
                          cuts = 0:12, 
...
if(do_MODIS_LAI){
  ...
  if(file.exists(full_file_path)) {
    ...
    # Calculate the slope (still in grams)
    all_GPP_Trends[["MODIS"]] <- calcLinearTrend(MODIS_full_Field)
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(guess_var, ".out.gz")))) {
      ...
      ### take sum of month and average of years and store for later use - very easy ###
      all_LAI_Map_Fields_list[[this_sim_Source@id]] <- aggregateYears(aggregateSubannual(this_simulation_LAI, target = "Year", method = "max"),  "mean")
      
      ### take yearly average and store for later use - very easy ###
      all_LAI_Seasonal_Fields_list[[this_sim_Source@id]] <- aggregateYears(this_simulation_LAI, "mean")
      ...
      # store the Field in the list of all Fields for plotting later
      all_LAI_Trend_Fields_list[[this_sim_Source@id]] <- this_LAI_Trend
...
if(do_MODIS_LAI){
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    # make comparison 
    message(paste("Comparison stats for run", this_sim_Source@name, "LAI values vs. MODIS LAI values"))
    suppressWarnings(
      spatial_comparisons_list[[this_sim_Source@id]] <- compareLayers(field1 = all_LAI_Map_Fields_list[[this_sim_Source@id]],
...
if(do_MODIS_LAI) {
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    # make comparison 
    message(paste("Comparison stats for run", this_sim_Source@name, "LAI trend vs. MODIS LAI trend"))
    suppressWarnings(
      trend_comparisons_list[[this_sim_Source@id]] <- compareLayers(field1 = all_LAI_Trend_Fields_list[[this_sim_Source@id]],
...
if(do_MODIS_LAI) {
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    # make comparison 
    message(paste("Comparison stats for run", this_sim_Source@name, "LAI seasonality vs. MODIS seasonality trend"))
    suppressWarnings(
      seasonal_comparisons_list[[this_sim_Source@id]] <- compareLayers(field1 = all_LAI_Seasonal_Fields_list[[this_sim_Source@id]],
...
if(do_GFED4_BA){
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(guess_var, ".out.gz")))) {
      ...
      rm(temp_fld)
      summary_table_lines[[this_sim_Source@name]] <- signif(as.numeric(all_Totals_vec[[this_sim_Source@name]]))
