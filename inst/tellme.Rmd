...
if(do_GOSIF_GPP){
  ...
  if(file.exists(full_file_path)) {
    ...
    GOSIF_full_Field@data <- GOSIF_full_Field@data[agpp != -9999.0,]
    GOSIF_full_Field <- layerOp(GOSIF_full_Field, operator = "divc", constant = 1000, layers = "agpp", new.layer = "agpp")
  ...
  for(this_simulation_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(LPJ_GUESS_variable, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(LPJ_GUESS_variable, ".out.gz")))) {
      ...
      # read the full data and and select the Total layer only
      suppressWarnings(
      ...
      all_GPP_Map_Fields_list[[this_simulation_Source@id]] <- aggregateYears(this_simulation_GPP, "mean")
      ...
      suppressWarnings(fitted_trends_dt <- this_simulation_GPP_dt[, as.list(summary(lm(agpp~Year))$coefficients[2,c(1,4)]), by = .(Lon, Lat)])
...
Awaiting example runs with monthly data...
