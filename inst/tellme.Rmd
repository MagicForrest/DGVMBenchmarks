...
params:
  new_directory:
    value: "/home/mforrest/GuessRuns/Tellus_dev/outputRelease4.1_r10118full/"
  new_name:
    value: "New Run"
  old_directory:
    value: "/home/mforrest/GuessRuns/Tellus_dev/output9957full/"
  old_name:
    value: "Old Run"
  data_directory: 
...
LPJ_GUESS_variable <- "agpp"

first_year_GOSIF <- 2001
last_year_GOSIF <- 2018

# for the fulle monthly data
...
if(do_GOSIF_GPP && file.exists(full_file_path)) {
  
  # Read the GOSIF monthly data via a GUESS formatted Source
  GOSIF_Source <- defineSource(id = "GOSIF",
                               name = "GOSIF",
                               dir = file.path(params$data_directory, GOSIF_GPP_monthly_dir),
                               format = GUESS)
  GOSIF_monthly_Field <- getField(source = GOSIF_Source,
                                  var = LPJ_GUESS_variable,
                                  file.name = GOSIF_GPP_monthly_file.name)
  
  # delete values with -9999
  GOSIF_monthly_Field@data <- GOSIF_monthly_Field@data[agpp != -9999.0,]
  
  # convert to kg
  GOSIF_monthly_Field <- layerOp(GOSIF_monthly_Field, operator = "divc", constant = 1000, layers = "agpp", new.layer = "agpp")
  
  # make the annual map for comparison
  GOSIF_map_Field <- aggregateYears(aggregateSubannual(GOSIF_monthly_Field, method = "sum", target = "Year"), method = "mean")
  
  
  # make a list of all Fields to be compared  (this will also include whatever model runs are available)
  all_GPP_Map_Fields_list <- list("GOSIF" = GOSIF_map_Field)
  ...
  for(this_simulation_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(LPJ_GUESS_variable, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(LPJ_GUESS_variable, ".out.gz")))) {
      ...
      # read the data and and select the Total layer only
      suppressWarnings(
      ...
      this_simulation_GPP <- selectLayers(this_simulation_GPP, "Total")
      renameLayers(this_simulation_GPP, LPJ_GUESS_variable)
      
      # take yearly average
      this_simulation_GPP <- aggregateYears(this_simulation_GPP, "mean")
      
      # store the Field in the list of all Fields for plotting later
      all_GPP_Map_Fields_list[[this_simulation_Source@id]] <- this_simulation_GPP
      
      # make comparison 
      message(paste("Comparison stats for run", this_simulation_Source@name, "GPP values vs. GOSIF GPP values"))
      suppressWarnings(
        suppressMessages(
          model_data_comparisons_list[[this_simulation_Source@id]] <- compareLayers(field1 = this_simulation_GPP,
                                                                                    field2 = GOSIF_map_Field,
  ...
  GPP_diff_values_plot <- plotSpatialComparison(model_data_comparisons_list, 
                                                ncol = 1, 
...
if(do_GOSIF_GPP) {
  ...
  GOSIF_Field@id <- makeFieldID(GOSIF_Field)
  
  
  
  # make a list of all Fields to be compared  (this will also include whatever model runs are available)
  all_GPP_Fields_list <- list("GOSIF" = GOSIF_Field)
  ...
  for(this_simulation_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(LPJ_GUESS_variable, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(LPJ_GUESS_variable, ".out.gz")))) {
      ...
      # calculate the linear trend and p-value for every gridcell     
      this_simulation_GPP_dt <- this_simulation_GPP@data
      # suppress warnings about "essentially perfect fit"
      suppressWarnings(fitted_trends_dt <- this_simulation_GPP_dt[, as.list(summary(lm(Total~Year))$coefficients[2,c(1,4)]), by = .(Lon, Lat)])
      ...
      # store the Field in the list of all Fields for plotting later
      all_GPP_Fields_list[[this_simulation_Source@id]] <- this_simulation_GPP
  ...
  # plot all slope values together
  GPP_trend_plot <- plotSpatial(all_GPP_Fields_list, 
                                layers = "Slope", 
                                ncol = 1, 
  ...
  GPP_trend_diff_plot <-  suppressWarnings(
    plotSpatialComparison(model_data_comparisons_list, 
                          ncol = 1, 
                          #cuts = c(-1.0, -0.5, -0.2, -0.1, -0.05, -0.02, -0.01, 0, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0),
                          drop.cuts = FALSE,
