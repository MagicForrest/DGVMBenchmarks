...
# this table is completely empty, will by built benchmark-by-benchmark
...
if(do_GCP_NBP){
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, "cflux.out")) || file.exists(file.path(this_benchmark_run_dir, "cflux.out.gz"))) {
      ...
      all_sim_full_NBP[[this_sim_Source@id]] <- this_simulation_NBP
      ...
      this_simulation_NBP_agg <- layerOp(x = this_simulation_NBP_agg, operator = "divc", layers = layers(this_simulation_NBP_agg), new.layer = layers(this_simulation_NBP_agg), constant = -1E12)
      
      # store the Field in the list of all Fields for plotting later
      all_NBP_Fields_list[[this_sim_Source@id]] <- this_simulation_NBP_agg
  ...
  # TODO calculate R^2 on this data
  ...
  if(!is.null(spatial_extent)) {
    ...
    for(this_sim_Source in all_simulation_Sources_list) {
      ...
      if (!inherits(possible.error, "try-error") && !is.data.table(spatial.extent)) {
        this_simulation_NBP_subset <- crop(x =  all_sim_full_NBP[[this_sim_Source@id]], y = spatial_extent, spatial.extent.id = spatial_extent_id)  
      }
      # else check if some gridcells to be selected with getGridcells
        ...
        this_simulation_NBP_subset <- selectGridcells(x = all_sim_full_NBP[[this_sim_Source@id]], gridcells = spatial_extent, spatial.extent.id = spatial_extent_id)
      }

      # aggregate with weighted sum and adjust units
      this_simulation_NBP_subset_agg <- aggregateSpatial(this_simulation_NBP_subset, method = "w.sum", lon_centres = hd_lons, lat_centres = hd_lats)
      print(this_simulation_NBP_subset_agg)
      
      this_simulation_NBP_subset_agg <- layerOp(x = this_simulation_NBP_subset_agg, operator = "divc", layers = layers(this_simulation_NBP_subset_agg), new.layer = layers(this_simulation_NBP_subset_agg), constant = -1E12)
  ...
  # calculate the NBP sum of the subset only
  stop()
}
