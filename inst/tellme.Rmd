 ...
 metric_table <- rbind(metric_table, 
  ...
  if(!is.null(spatial_extent)) {
    ...
    for(this_sim_Source in all_simulation_Sources_list) {
      ...
      this_simulation_NBP_subset_agg <- layerOp(x = this_simulation_NBP_subset_agg, operator = "mulc", layers = layers(this_simulation_NBP_subset_agg), new.layer = layers(this_simulation_NBP_subset_agg), constant = -KG_TO_PG)
...
``` {r Soil N fluxes table, echo=FALSE, out.width = '100%', out.height= '150%'}
this_complete_df <- modelSummaryTable(simulations = all_simulation_Sources_list, 
                                      var = "soil_nflux", 
                                      periods = summary_periods, 
                                      new_sim =  params$new_name, 
