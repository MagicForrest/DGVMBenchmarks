...
# Settings
To see the settings for this run, click the "Code" button to the right ------------>
```{r settings, echo = TRUE, class.source = 'fold-hide'}
...
if(do_global_LAI){
  ...
  if(length(LAI_data_Fields) == 0) {
    ...
    for(this_data_Field in this_benchmark@datasets) {
      ...
      # TODO, make this more flexible for multiple datasets
      data_LAI_PgC <- areaWeightedTotal(all_LAI_Maps[[this_data_Field@source@name]])@data * M2_TO_MegaM2
    ...
    for(this_sim_Source in all_simulation_Sources_list) {
      ...
      if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
        ...
        # calculate and save the global total
        this_LAI_PgC <- areaWeightedTotal(all_LAI_Maps[[this_sim_Source@name]])@data * M2_TO_MegaM2
...
if(do_global_BA){
  ...
  if(length(BA_data_Fields) == 0) {
    ...
    for(this_data_Field in this_benchmark@datasets) {
      ...
      # make and save the mean annual global LAI
      # TODO, make this more flexible for multiple datasets
      data_BA_MHa <- areaWeightedTotal(all_BA_Maps[[this_data_Field@source@name]])@data * M2_TO_Mha
    ...
    for(this_sim_Source in all_simulation_Sources_list) {
      ...
      if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
        ...
        # read the full data and
        suppressWarnings(
          this_simulation_BA <- getField(source = this_crop_global_Source,
                                         this_benchmark@guess_var,
                                         first.year = this_benchmark@first.year,
                                         last.year = this_benchmark@last.year,
                                         quick.read = quick_read,
        ...
        # calculate and save the global total
        this_BA_Mha <- areaWeightedTotal(all_BA_Maps[[this_sim_Source@name]])@data * M2_TO_Mha
...
if(do_Pan_Biomass){
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(guess_var, ".out.gz")))) {
      ...
      # save
      all_Model_Biomass_Fields[[this_crop_global_Source@name]]  <- this_simulation_Field
  ...
  for(this_row_number in 1:nrow(pan_2007_dt)) {
    ...
    for(this_sim_Source in all_simulation_Sources_list) {
      ...
      this_sim_region_summed <- aggregateSpatial(this_sim_region, method = "w.sum")    
...
if(do_Regrowth){
  ...
  # now the data
  regrowth_file <- file.path(input_dir, "benchmark_regrowth_and_eq_dynamics_251122.csv")
  regrowth <- read.csv(file =regrowth_file)
  
  
  regrowth_full_dt <- data.table(regrowth)[, list(bin_ranges, bin_num, AGB_kgCm2_med, AGB_kgCm2_10, AGB_kgCm2_90, Biome, final)]
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
      ...
      tmp <- inner_join(this_simulation_regrowth@data,gridlist)
      tmp_all <- copy(tmp)
      
      
      #### Only Boreal for original plots
      tmp <- tmp[which(tmp$Biome =="Boreal")] # not working for Tropical until "cru-compatibility" in gridlist is implemented.
      
      #introduce some new variables, Age, age-bins and summary stats:
      tmp <-tmp %>% select(Lat,Lon,Year,Total,Forest_sum,Natural_sum,distyear,Biome)
      tmp_regr <- tmp %>% group_by(Lat,Lon) %>% filter(Year >= distyear)
      age <- tmp_regr %>% group_by(Lat,Lon) %>% mutate(Age = Year - distyear)
      age_bins <- age %>% group_by(Lat,Lon) %>% mutate(age_bin = cut(Age, breaks=seq(0,300,10),include.lowest = TRUE,ordered_result = TRUE))
      
      #summary stats on bins:
      
      # Boreal only
      regrowth_mod <- age_bins %>% group_by(age_bin) %>% summarise(AGB_kgCm2_med = median(Total),
                                                                   AGB_kgCm2_10 = quantile(Total, c(.10) ),
                                                                   AGB_kgCm2_90 = quantile(Total, c(.90) ),
                                                                   bin_centres =  floor(mean(Age)))
      
      regrowth_mod_full <- age_bins %>% group_by(age_bin, Biome) %>% summarise(AGB_kgCm2_med = median(Total),
                                                                               AGB_kgCm2_10 = quantile(Total, c(.10) ),
                                                                               AGB_kgCm2_90 = quantile(Total, c(.90) ),
                                                                               bin_centres =  floor(mean(Age)))
      
      #visualise aggregated output:
      plot(regrowth_mod$age_bin,
           regrowth_mod$AGB_kgCm2_med,
           ylim=c(0,35),col="red",xlim=c(0,16),ylab='',xlab='',
           xaxt = 'n',yaxt= 'n')
      
      abline(h=10,col="light grey")
      arrows(1:30, regrowth_mod$AGB_kgCm2_10, 1:30, regrowth_mod$AGB_kgCm2_90, length=0.00, angle=90, code=3,col= "dark green")
      #axis(side=1,at=1:12, labels=seq(10,240,20))
      mtext(side=2,"AGB KgCm2",line=1)
      ...
      regrowth_obs <-  regrowth[regrowth$Biome=="Temperate",]
      
      par(new=TRUE)
      plot(regrowth_obs$bin_num/10,
           regrowth_obs$AGB_kgCm2_med,
           ylim=c(0,35),col="red",xlim=c(0,16),ylab='',xlab='',
           xaxt = 'n',yaxt= 'n')
      
      arrows(regrowth_obs$bin_num/10, regrowth_obs$AGB_kgCm2_10, regrowth_obs$bin_num/10, regrowth_obs$AGB_kgCm2_90, length=0.00, angle=90, code=3,col= "red")
      ...
      #introduce some new variables, Age, age-bins and summary stats:
      tmp_all <- tmp_all %>% select(Lat,Lon,Year,Total,Forest_sum,Natural_sum,distyear,Biome)
      tmp_regr <- tmp_all %>% group_by(Lat,Lon) %>% filter(Year >= distyear)
      age <- tmp_regr %>% group_by(Lat,Lon) %>% mutate(Age = Year - distyear)
      age_bins <- age %>% group_by(Lat,Lon) %>% mutate(age_bin = cut(Age, breaks=seq(0,300,10),include.lowest = TRUE,ordered_result = TRUE))
...
```{r regrowth ggplots, echo=FALSE, out.width = '100%'}

#### Regrowth plots with ggplot ####
if(do_Regrowth){
  
  regrowth_plot <- ggplot(regrowth_full_dt, aes(x=bin_centres)) + geom_point(aes(y = AGB_kgCm2_med,
                                                                                 col = Source)) +
    geom_errorbar(aes(ymin=AGB_kgCm2_10, ymax=AGB_kgCm2_90, col = Source)) +
    facet_wrap(~Biome, ncol = 2) +
    labs(x= "Years after disturbance", y = expression(AGB~kgC~m^{"-2"})) +
...
if(do_Global_Biomass){
  ...
  if(file.exists(full_file_path)) {
    ...
    all_Biomass_totals[[this_data_Field@source@name]] <- areaWeightedTotal(this_data_Field)@data * KG_TO_PG
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
      ...
      # calculate the global total
      all_Biomass_totals[[this_sim_Source@name]] <-areaWeightedTotal(all_Biomass_Maps[[this_sim_Source@name]])@data * KG_TO_PG
