...
#### Here select the benchmarks to do ####
...
#### Here define the global script parameters ####
script_settings <- list(
  
  # version string (for making quick read files)
...
# for "reasons" this helps RMarkdown find the right Helvetica font when rendering from the commandline on some systems 
library(showtext) 
showtext_auto()
...
script_settings$simulation_sources <- list(old_simulation_Source, new_simulation_Source)

simulation_sources <- list(old_simulation_Source, new_simulation_Source)


# 
# # quick read switch and version label (for making quick read files)
# settings$quick_read <- TRUE
# settings$analysis_version <- "tellus_v1.2" 
# 
# # do verbose reads - this makes a lot of printed output which will appear in the report, only enable for developing/debugging the reading code
# settings$verbose_read <- FALSE
# 
# # do publication plots
# settings$doPublicationPlots <- FALSE
# 
# # plot maps (turn off to save time)
# settings$doMapPlots <- TRUE
# 
# # Summary periods
# settings$summary_periods <- list(c(1961, 1990),
#                         c(1990, 2000))
# 
# 
# #  **** SPECIAL SUBSETTING FOR STEFAN'S GLOBAL RETUNING  ****
...
# # Standard plot formatting arguments
# settings$num_cols <- 1
# settings$map_overlay <- "coastlines"
# settings$stats_lon <- -130
# settings$stats_lat <- -20
...
version_label <- paste(settings$analysis_version, settings$spatial_extent_id, sep = "_")
...
if(do_GCP_NBP){
  ...
  overall_tables <- benchmark_GCP(simulation_sources = simulation_sources,
                                  settings = script_settings,
                                  params = params,
...
if(do_global_GPP){
  ...
  if(length(GPP_data_Fields) == 0) {
    ...
    overall_tables[["totals"]] <- rbind(overall_tables[["totals"]], summary_table_lines)
    
    # make mini metric table for this benchamrk and append to overall table
    overall_tables[["metrics"]] <- rbind(overall_tables[["metrics"]], 
...
### Absolute values
```{r}
if(do_global_GPP & settings$doMapPlots){  
  ...
  GPP_abs_values_plot  <- GPP_abs_values_plot  + geom_text(data = total_label_df,  mapping = aes(x = x, y = y, label = label), size = settings$map_annotation_text_size)
    ...
    for(comparison_name  in names(all_GPP_comparisons[["Values"]])) {
      metric_label_df <- rbind(metric_label_df,
                               list(label = paste(this_metric, "=", round(all_GPP_comparisons[["Values"]][[comparison_name]]@stats[[this_metric]], 2)), 
                                    ...
                                    x = settings$stats_lon, y = settings$stats_lat - lat_offset),
                               stringsAsFactors = FALSE)  
    }
    metric_label_df$Facet <- factor(metric_label_df$Facet, levels = names(all_GPP_comparisons[["Values"]]))
    GPP_diff_values_plot  <- GPP_diff_values_plot  + geom_text(data = metric_label_df,  mapping = aes(x = x, y = y, label = label), size = settings$map_annotation_text_size)
...
if(do_global_LAI){
  ...
  if(length(LAI_data_Fields) == 0) {
    ...
    names(summary_table_lines) <- names(overall_tables[["totals"]])
    overall_tables[["totals"]] <- rbind(overall_tables[["totals"]], summary_table_lines)
...
### Absolute values
```{r}
if(do_global_LAI & settings$doMapPlots){  
  # plot all absolute values together
  ...
  LAI_abs_values_plot  <- LAI_abs_values_plot  + geom_text(data = total_label_df,  mapping = aes(x = x, y = y, label = label), size = settings$map_annotation_text_size)
...
if(do_global_LAI & settings$doMapPlots){  
  ...
  for(this_metric in this_benchmark@metrics) {
    ...
    LAI_diff_values_plot  <- LAI_diff_values_plot  + geom_text(data = metric_label_df,  mapping = aes(x = x, y = y, label = label), size = settings$map_annotation_text_size)
    lat_offset <- lat_offset + 10  
  }
...
### Absolute values
```{r}
if(do_global_BA & settings$doMapPlots){  
  # plot all absolute values together
  ...
  BA_abs_values_plot  <- BA_abs_values_plot  + geom_text(data = total_label_df,  mapping = aes(x = x, y = y, label = label), size = settings$map_annotation_text_size)
...
if(do_global_BA & settings$doMapPlots){  
  ...
  for(this_metric in this_benchmark@metrics) {
    ...
    BA_diff_values_plot  <- BA_diff_values_plot  + geom_text(data = metric_label_df,  mapping = aes(x = x, y = y, label = label), size = settings$map_annotation_text_size)
    lat_offset <- lat_offset + 10  
  }
...
if(do_Regrowth){
  ...
  for(this_sim_Source in simulation_sources) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
      ...
      model_joined <- inner_join(this_simulation_regrowth@data, gridlist, relationship = "many-to-many")
...
```{r fluxnet, echo=FALSE, out.width = '100%'}


#### FLUXNET Data Prep ####
if(do_FLUXNET){
  
  all_fluxnet_fluxes <-c("GPP", "NEE", "Reco")
  
  # read the FLUXNET data
  fluxnet_src <- defineSource(id = "FLUXNET",
                              format = FLUXNET,
                              dir ="/data/shared/Tellus/FLUXNET_sites/")
  all_fluxnet_temp_Fields <- list()
  for(this_flux_name in all_fluxnet_fluxes) {
    print(this_flux_name)
    all_fluxnet_temp_Fields[[this_flux_name]] <- getField(fluxnet_src, quant = this_flux_name)
    #print(all_fluxnet_temp_Fields)
    #print(plotTemporal(all_fluxnet_temp_Fields))
  }
  
  
  # combine all requested variables into one Field
  FLUXNET_data_field <- copy(all_fluxnet_temp_Fields[[1]])
  if(length(names(all_fluxnet_temp_Fields)) > 1){
    for(this_temp_flux in names(all_fluxnet_temp_Fields)[2:length(names(all_fluxnet_temp_Fields))]){
      FLUXNET_data_field <- copyLayers(from = all_fluxnet_temp_Fields[[this_temp_flux]], to = FLUXNET_data_field, layer.names = this_temp_flux )
  ...
  this_benchmark <- new("Benchmark",
                        ...
                        description = "FLUXNET",
                        simulation = "fluxnet",
                        guess_var = c("dgpp", "dnee", "dreco"),
                        guess_layers = all_fluxnet_fluxes,
                        unit = "kgC/m^2",
                        agg.unit = "Not Used",
                        datasets = list("FLUXNET" = FLUXNET_data_field),
                        first.year = 0,
                        last.year = 150,
                        metrics = character(0))
  
  # # read the model output
  all_FLUXNET_Fields <- list()
  all_FLUXNET_Fields[["FLUXNET"]] <- FLUXNET_data_field
  #  # # loop through model runs to be processed
  
  for(this_sim_Source in simulation_sources) {
    
    this_sim_all_layers_list <- list()
    for(this_fluxnet_flux in this_benchmark@guess_layers) {
      
      if(this_fluxnet_flux == "GPP") {
        this_guess_var <- "dgpp"
        this_guess_layer <- "Total"
      }
      else if(this_fluxnet_flux == "NEE") {
        this_guess_var <- "dnee"
        this_guess_layer <- "NEE"
      }
      else if(this_fluxnet_flux == "Reco") {
        this_guess_var <- "dreco"
        this_guess_layer <- "Reco"
      }
      
      
      # check if file is present (if not don't include this run)
      this_benchmark_run_dir <- file.path(this_sim_Source@dir,this_benchmark@simulation)
      if(file.exists(file.path(this_benchmark_run_dir, paste0(this_guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_guess_var, ".out.gz")))) {
        
        # make local sources pointing to the simulation subrun directory
        this_FLUXNET_Source <- this_sim_Source
        this_FLUXNET_Source@dir <- this_benchmark_run_dir
        
        for(this_flux_name in all_fluxnet_fluxes){
          
          # read the full data and
          suppressWarnings(
            this_simulation_FLUXNET <- getField(source = this_FLUXNET_Source,
                                                this_guess_var,
                                                layers = this_guess_layer,  
                                                verbose = settings$verbose_read,
                                                quick.read = settings$quick_read,
                                                quick.read.file = paste(this_guess_var, version_label, sep = "_"))
          )
          renameLayers(this_simulation_FLUXNET, this_fluxnet_flux)
          
        }
        this_sim_all_layers_list[[this_fluxnet_flux]] <- this_simulation_FLUXNET
        
      } # if output file exists
      
    } # for each LPJ-GUESS variable
    
    
    
    # combine the individual modelled Fields (for GPP, NEE and Reco) into a single Field and save it 
    this_sim_field <- copy(this_sim_all_layers_list[[1]])
    if(length(names(this_sim_all_layers_list)) > 1){
      for(this_temp_flux in names(this_sim_all_layers_list)[2:length(names(this_sim_all_layers_list))]){
        this_sim_field <- copyLayers(from = this_sim_all_layers_list[[this_temp_flux]], to = this_sim_field, layer.names = this_temp_flux )
      }
    }
    
    # some lons and lats don't match from the LPJ-GUESS fluxnet gridlist with the FLUXNET data, so manually correct them here 
    correction_df <- data.frame(
      site = c("CH-Lae", "DE-Gri", "DE-Hai", "DE-Tha", "FI-Sod"),
      lons_old = c(8.3650, 13.5126, 10.4530, 13.5652, 26.6378 ),
      lons_new = c(8.36439, 13.51253, 10.45217, 13.56515, 26.63859),   
      lats_old = c(47.4781, 50.9500, 51.0792, 50.9624, 67.3619),
      lats_new = c(47.47833, 50.94947, 51.07921, 50.96256, 67.36239)
    )    
    
    temp_dt <- copy(this_sim_field@data)
    print(sort(unique(temp_dt[["Lon"]])))
    prev <- sort(unique(temp_dt[["Lon"]]))
    for(this_rowindex in 1:nrow(correction_df)){
      #print( temp_dt[ Lon == correction_df[this_rowindex, "lons_old"] & Lat == correction_df[this_rowindex, "lats_old"], ])
      # 
      temp_dt[ Lon == correction_df[this_rowindex, "lons_old"] & Lat == correction_df[this_rowindex, "lats_old"],  c("Lon", "Lat") := list(correction_df[this_rowindex, "lons_new"], correction_df[this_rowindex, "lats_new"] )]
      
      
    }
    print(sort(unique(temp_dt[["Lon"]])))
    print(sort(unique(temp_dt[["Lon"]])) - prev)
    this_sim_field@data <- temp_dt
    
    all_FLUXNET_Fields[[this_sim_Source@name]] <- this_sim_field
  ...
  print(all_FLUXNET_Fields)
  
  # print(plotTemporal(all_FLUXNET_Fields, layers = "GPP", col.by = "Source", linetype.by = "Layer"))
  
  
  # round all lons and lats in the data 4 decimal places
  decimal_places <- 3
  temp_data_Field <- all_FLUXNET_Fields[["FLUXNET"]]
  temp_data_Field_bup <- copy(temp_data_Field)
  temp_data_Field <- copy(temp_data_Field_bup)
  temp_data_Field@data <- temp_data_Field@data[ , Lon := round(Lon, decimal_places)]
  temp_data_Field@data <- temp_data_Field@data[ , Lat := round(Lat, decimal_places)]
  temp_data_Field@spatial.extent <- temp_data_Field@spatial.extent[ , Lon := round(Lon, decimal_places)]
  temp_data_Field@spatial.extent <- temp_data_Field@spatial.extent[ , Lat := round(Lat, decimal_places)]
  all_FLUXNET_Fields[["FLUXNET"]] <- temp_data_Field
  
  temp_data_Field <- all_FLUXNET_Fields[["Old Run"]]
  temp_data_Field_bup <- copy(temp_data_Field)
  temp_data_Field <- copy(temp_data_Field_bup)
  temp_data_Field@data <- temp_data_Field@data[ , Lon := round(Lon, decimal_places)]
  temp_data_Field@data <- temp_data_Field@data[ , Lat := round(Lat, decimal_places)]
  all_FLUXNET_Fields[["Old Run"]] <- temp_data_Field
  
  temp_data_Field <- all_FLUXNET_Fields[["New Run"]]
  temp_data_Field_bup <- copy(temp_data_Field)
  temp_data_Field <- copy(temp_data_Field_bup)
  temp_data_Field@data <- temp_data_Field@data[ , Lon := round(Lon, decimal_places)]
  temp_data_Field@data <- temp_data_Field@data[ , Lat := round(Lat, decimal_places)]
  all_FLUXNET_Fields[["New Run"]] <- temp_data_Field
  
  
  for(this_name in names(all_FLUXNET_Fields)){
    
    
    # CH-Lae, DE-Gri, DE-Tha, DK-Sor, FI-Hyy, FI-Sod, FR-Fon, FR-LBr, FR-Peu, IT-Col, IT-Lav, IT-MBo, IT-Noe, IT-Ren, IT-Sro, RU-Cok, RU-Fyo, US-Blo, US-Ha1, US-Los, US-MMS, US-Me2(??), US-NR1, US-PFa, 
    
    # Loop across all sites
    matched_sites <- 0
    for(site_number in 1:nrow(all_FLUXNET_Fields[["FLUXNET"]]@spatial.extent)){
      
      this_site <- all_FLUXNET_Fields[["FLUXNET"]]@spatial.extent[site_number ,]
      print(this_site$Code)
      
      matched <- 0
      for(this_Field in all_FLUXNET_Fields){
        tada <- selectGridcells(this_Field, c(this_site$Lon, this_site$Lat), spatial.extent.id = this_site$Name)@data
        if(nrow(tada) > 0 ) {
          matched <- matched +1
        }
      }
      ...
      if(matched ==3){
        print("   ******  Got model")
        matched_sites <- matched_sites +1
      }
      
      
      # print(plotTemporal(all_FLUXNET_Fields, layers = "GPP", gridcells = c(this_site$Lon, this_site$Lat)))
      
    }
    print(paste("Numbers sites with model data = ", matched_sites))
    # now plot...
...
### Absolute values
```{r}
if(do_Global_Biomass & settings$doMapPlots){  
  # plot all absolute values together
  ...
  Biomass_abs_values_plot  <- Biomass_abs_values_plot  + geom_text(data = total_label_df,  mapping = aes(x = x, y = y, label = label), size = settings$map_annotation_text_size)
...
if(do_Global_Biomass & settings$doMapPlots){  
  ...
  for(this_metric in this_benchmark@metrics) {
    ...
    Biomass_diff_values_plot  <- Biomass_diff_values_plot  + geom_text(data = metric_label_df,  mapping = aes(x = x, y = y, label = label), size = settings$map_annotation_text_size)
    lat_offset <- lat_offset + 10  
  }
...
if(do_Global_Summary_Tables) {
  this_complete_df <- modelSummaryTable(simulations = simulation_sources, 
                                        settings = settings,
                                        var = "cflux", 
                                        periods = settings$summary_periods, 
                                        new_sim =  params$new_name, 
                                        old_sim = params$old_name, 
                                        spatial_extent = settings$spatial_extent,
                                        spatial_extent_id = settings$spatial_extent_id,
                                        area_unit = "m^2",
...
# retrieve the metrics table from the list
summary_table <- overall_tables[["totals"]]


# get the global soil N2O flux for the summary table
# for 2000-2010 to compare to the IPCC AR5 values
this_soil_N2O_df <- modelSummaryTable(simulations = simulation_sources, 
                                      settings = settings,
                                      var = "soil_nflux", 
...
summary_N2O_line$Quantity <- "Soil N~2~O Emissions"
summary_N2O_line$Unit <- "TgN y^-1^"
...
# retrieve the metrics table from the list
metric_table <- overall_tables[["metrics"]]
