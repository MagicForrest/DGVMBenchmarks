...
#       2. The paths defined in the "dir" argument only point to the overall benchmark directory.  To actually acess one of the benchamrk runs (ie crop_global) you should copy these Source objects and manually append the benchmark directory name to the "dir.  See the GCP code block for an example.
...
Also note that the table above it just an example with the default formatting.  I have not yet looked into making prettier tables, but it is definitely psossible, see here: [https://rmarkdown.rstudio.com/lesson-7.html](https://rmarkdown.rstudio.com/lesson-7.html)
