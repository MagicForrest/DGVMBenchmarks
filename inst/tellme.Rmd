...
if(do_global_BA){
  ...
  GFED4_BA_monthly_dir <- "monthly_ba/GFED4"
...
if(do_Pan_Biomass){
  ...
  kable(final_dt, digits = 1, align = "l")
...
if(do_Global_Biomass){
  ...
  if(file.exists(full_file_path)) {
    ...
    all_Map_Fields_list[[liu_biomass$dataset_name]] <- this_data_Field
  }
  ...
  for(this_sim_Source in all_simulation_Sources_list) {
    ...
    if(file.exists(file.path(this_benchmark_run_dir, paste0(this_benchmark@guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_benchmark@guess_var, ".out.gz")))) {
      ...
      this_cmass <- copyLayers(from = final_lc_dt, to = this_cmass, layer.names = c("NATURAL", "PASTURE", "CROPLAND"))
      ...
      this_cmass <- layerOp(this_cmass, "*", c("CROPLAND", "Cropland_sum"),  "Cropland_sum")
      this_cmass <- layerOp(this_cmass, "+", c("Natural_sum", "Pasture_sum", "Cropland_sum"),  "Total_ABG")
      all_Map_Fields_list[[this_sim_Source@id]] <- selectLayers(this_cmass, "Total_ABG")
  ...
  print(all_Map_Fields_list)
  
  print(plotSpatial(all_Map_Fields_list))
